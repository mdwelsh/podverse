...
async function currentSubscription(): Promise<Subscription | undefined> {
  const { userId } = auth();
  const supabase = await getSupabaseClient();
  if (userId) {
    try {
      const existingSubscriptions = (await GetSubscriptions(supabase, userId)).filter(
        (s) => s.state === SubscriptionState.Active || s.state === SubscriptionState.CancelPending,
      );
      if (existingSubscriptions.length > 1) {
        return existingSubscriptions.filter((s) => s.state === SubscriptionState.Active)[0];
      }
...
function PlanCard({
  ...
  const isCurrent = existingPlan.id === plan.id;
  const endDate =
  ...
  return (
    <div className={`flex flex-col gap-1 ${isCurrent ? 'text-muted-foreground' : 'text-white'}`}>
      ...
      <div className="bg-muted flex flex-col gap-2 p-4 h-[400px] lg:h-[300px]">
        ...
        <div className="py-2 mx-auto lg:mx-0 text-xs h-3 text-muted-foreground">{plan.price > 0 && 'Cancel any time'}</div>
      </div>
    </div>
    </div>
  );
