    ...
    return (
        <div className="w-full flex flex-col gap-4">
            <div className="text-muted-foreground">
                From <Link href={`/podcast/${episode.podcast.slug}`} > <span className="text-primary">{episode.podcast.title}</span></Link>
            </div>
...
function ParagraphView({ paragraph }: { paragraph: any }) {
    ...
    const startString = `${startHours}:${startMinutes.toString().padStart(2, '0')}:${startSeconds.toString().padStart(2, '0')}`;
...
function TranscriptView({ transcript }: { transcript: any }) {
    ...
    const views = paragraphs.map((paragraph: any, index: number) => (
        <ParagraphView paragraph={paragraph} key={index} />
    ));
