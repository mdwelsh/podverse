...
export async function SpeakerIDEpisode(episodeId: number): Promise<string> {
  ...
  for (const [speakerId, name] of Object.entries(speakers)) {
    ...
    await UpdateSpeakerMap(supabase, episodeId, speakerId, name);
  }
  console.log(`Done speaker ID for episode ${episodeId}`);
  return `SpeakerID on episode ${episodeId} done`;
}

export async function EmbedEpisode(episodeId: number): Promise<string> {
  ...
  const res = await fetch(episode.transcriptUrl);
  const text = await res.text();
  const pageId = await Embed({ text, episode, podcast });
