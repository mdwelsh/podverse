...
CREATE OR REPLACE FUNCTION "public"."chunk_vector_search"("embedding" "public"."vector", "match_threshold" double precision, "match_count" integer, "min_content_length" integer) RETURNS TABLE("id" bigint, "page_id" bigint, "slug" "text", "heading" "text", "content" "text", "similarity" double precision)
    LANGUAGE "plpgsql"
    AS $$-- This function added by mdw in March 2024.
...
begin
  ...
  -- We only care about chunks that have a useful amount of content
...
ALTER TABLE ONLY "public"."Documents"
    ADD CONSTRAINT "public_Documents_episode_fkey" FOREIGN KEY ("episode") REFERENCES "public"."Episodes"("id");

ALTER TABLE ONLY "public"."Jobs"
    ADD CONSTRAINT "public_Jobs_episode_fkey" FOREIGN KEY ("episode") REFERENCES "public"."Episodes"("id") ON UPDATE CASCADE ON DELETE CASCADE;
...
CREATE POLICY "Allow insert by Podcast owner" ON "public"."Chunks" FOR INSERT WITH CHECK ("public"."is_podcast_owner"(( SELECT "d"."episode"
  ...
  WHERE ("d"."episode" = "d"."episode")), "public"."requesting_user_id"()));

CREATE POLICY "Allow insert by Podcast owner" ON "public"."SpeakerMap" FOR INSERT WITH CHECK (("public"."requesting_user_id"() IN ( SELECT "p"."owner" AS "user_id"
   FROM (("public"."SpeakerMap" "sm"
     JOIN "public"."Episodes" "e" ON (("sm"."episode" = "e"."id")))
     JOIN "public"."Podcasts" "p" ON (("e"."podcast" = "p"."id")))
  WHERE ("e"."id" = "sm"."episode")
 LIMIT 1)));
...
CREATE POLICY "Allow insert or update by Podcast owner" ON "public"."SpeakerMap" FOR UPDATE USING (("public"."requesting_user_id"() IN ( SELECT "p"."owner" AS "user_id"
   FROM (("public"."SpeakerMap" "sm"
     JOIN "public"."Episodes" "e" ON (("sm"."episode" = "e"."id")))
     JOIN "public"."Podcasts" "p" ON (("e"."podcast" = "p"."id")))
  WHERE ("e"."id" = "sm"."episode")
...
CREATE POLICY "Enable insert for Podcast owner" ON "public"."Documents" FOR INSERT WITH CHECK ("public"."is_podcast_owner"("episode", "public"."requesting_user_id"()));
...
CREATE POLICY "Enable read access for all users" ON "public"."SpeakerMap" FOR SELECT USING (true);
